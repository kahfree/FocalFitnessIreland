...
class Client extends BaseController
{
	...
	public function saveCustomWorkout($ExerciseList,$total_workout_duration,$exerciseDuration){
		
		$data = [];

		$ExerciseList = explode(",",$ExerciseList);
		$exerciseDuration = explode(",",$exerciseDuration);
		//print_r($ExerciseList);
		//echo 'total '.$total_workout_duration;
		//echo 'count of exercise list' . count($ExerciseList);
		//Create new workout and save to workout table
		$workoutModel = new WorkoutModel();
		$clientModel = new ClientModel();
		$workoutName = $clientModel->getClientByID(session()->get('clientID'));
		$workoutName = $workoutName->FirstName .' '. $workoutName->LastName ."'s workout";
		$newData = [
			'Name' => $workoutName,
			'TotalDuration' => ($total_workout_duration*60),
			'UserMade' => 1,
			'Featured' => 0
		];
		$workoutModel->save($newData);

		//Add all exercises done in workout to 'exercises_in_workout' table
		$exercisesInWorkoutModel = new ExercisesInWorkoutModel();
		$workoutID = $workoutModel->getUserMadeWorkout();
		$newExerciseInWorkout = [];
		for($i = 0; $i < count($ExerciseList); $i++){
			$newExercise =[
				'WorkoutID' => $workoutID, 
				'ExerciseID' => $ExerciseList[$i],
				'ExerciseDuration' => $exerciseDuration[$i],
			];
			array_push($newExerciseInWorkout,$newExercise);
			
		}
		print_r($newExerciseInWorkout);
		$exercisesInWorkoutModel->insertBatch($newExerciseInWorkout);
		//Add the new workout to the client's saved workout library
		$savedWorkoutModel = new SavedWorkoutModel();
		$newData = [
			'ClientID' => session()->get('clientID'),
			'WorkoutID' => $workoutID
		];
		$savedWorkoutModel->save($newData);
		return redirect()->to('/savedWorkouts');
		$data['yeahboi'] = $newExerciseInWorkout;
		echo view('templates/clientheader',$data);
